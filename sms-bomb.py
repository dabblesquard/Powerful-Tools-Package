...
def send_iqra_simple(phone):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            failed += 1
            log("fail", f"SMS Send Failed | Code: {r.status_code}")
...
def send_ecourier_post(phone):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            failed += 1
            log("fail", f"eCourier-POST Failed | Code: {r.status_code}")
    except Exception as e:
        failed += 1
...
def send_ecourier_get(phone):
    ...
    try:
        ...
        if r.status_code == 200:
            success += 1
            log("success", f"eCourier-GET SMS Sent | Response: {r.text[:30]}...")
        else:
            failed += 1
            log("fail", f"eCourier-GET Failed | Code: {r.status_code}")
    except Exception as e:
        failed += 1
